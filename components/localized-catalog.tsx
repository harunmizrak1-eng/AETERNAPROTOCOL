    ...
    <div lang={locale} dir={ui.dir} className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-40 border-b border-hairline bg-white/95 px-6 py-4 backdrop-blur md:px-10">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <Link href="/" aria-label="ZPHC Türkiye ana sayfa"><Image src="/brand/zphc-logo.png" alt="ZPHC" width={250} height={42} className="h-8 w-auto" priority /></Link>
      ...
      <main id="main-content">
        <section className="border-b border-hairline bg-surface px-6 py-10 md:px-10 md:py-14">
          <div className="mx-auto max-w-7xl">
            ...
            <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={ui.searchHint} className="mt-7 min-h-12 w-full max-w-xl border border-hairline bg-white px-5 text-sm outline-none focus:border-gold" />
          </div>
        </section>
            ...
            <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {filtered.map((product) => {
                ...
                return <li key={product.slug}>
                  <Link href={`/urunler/${product.slug}`} className="group flex h-full flex-col border border-hairline bg-white p-3 shadow-[0_8px_28px_rgba(0,49,76,0.05)] transition hover:-translate-y-0.5 hover:border-gold/40">
                    {product.image && <div className="bg-surface p-2"><Image src={product.image} alt={product.name} width={360} height={360} className="aspect-square w-full object-contain" /></div>}
