...
function CartDrawer() {
  const { items, count, open, setOpen, change, remove } = useCart()
  const message = [
    "Merhaba, aşağıdaki ürünleri sipariş vermek istiyorum:",
    "",
    ...items.map((item) => `• ${item.quantity} × ${item.name}`),
    "",
    "Fiyat, stok durumunu ve teslimat bilgisini teyit eder misiniz?",
  ].join("\n")
  ...
  return <>
    ...
    <aside aria-label="Sipariş sepeti" aria-hidden={!open} className={`fixed inset-y-0 right-0 z-[80] flex w-[min(92vw,420px)] flex-col bg-white shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {!items.length ? <p className="rounded-xl bg-surface p-5 text-sm leading-6 text-muted-foreground">Ürünleri sepete ekleyin. Fiyat ve stok bilgisi WhatsApp üzerinden teyit edilir.</p> : <ul className="space-y-4">{items.map((item) => <li key={item.slug} className="grid grid-cols-[64px_1fr] gap-3 rounded-xl border border-hairline p-3">
          ...
          <div className="min-w-0"><p className="line-clamp-2 text-sm font-bold leading-5">{item.name}</p><div className="mt-2 flex items-center justify-between"><div className="flex items-center rounded-full border border-hairline"><button type="button" onClick={() => change(item.slug, item.quantity - 1)} className="h-8 w-8" aria-label="Azalt">−</button><span className="w-7 text-center text-xs font-bold">{item.quantity}</span><button type="button" onClick={() => change(item.slug, item.quantity + 1)} className="h-8 w-8" aria-label="Artır">+</button></div><button type="button" onClick={() => remove(item.slug)} className="text-xs font-semibold text-muted-foreground hover:text-red-600">Kaldır</button></div></div>
        </li>)}</ul>}
      </div>
      {items.length > 0 && <div className="border-t border-hairline bg-surface/60 p-5"><p className="mb-4 text-sm leading-6 text-muted-foreground">Fiyat ve stok bilgisi mesajınızdan sonra teyit edilir.</p><a href={whatsappLink(message)} target="_blank" rel="noopener noreferrer" onClick={() => track("WhatsApp Cart Checkout", { item_count: count })} className="flex min-h-12 w-full items-center justify-center rounded-full bg-[#25D366] px-5 text-base font-bold text-white">WhatsApp’tan tamamla →</a><p className="mt-3 text-center text-xs leading-5 text-muted-foreground">Ödeme WhatsApp içinde yapılmaz; sipariş ve stok bilgisi resmî hatta teyit edilir.</p></div>}
    </aside>
  </>
}
