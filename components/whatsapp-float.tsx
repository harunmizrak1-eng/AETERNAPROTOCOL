...
export function WhatsappFloat() {
  ...
  return <div className={`fixed bottom-20 right-3 z-40 flex flex-col items-end gap-2.5 transition-all duration-200 sm:bottom-6 sm:right-6 ${footerVisible ? "pointer-events-none translate-y-3 opacity-0" : "opacity-100"}`} aria-hidden={footerVisible} aria-label="Resmî iletişim kanalları">
    <TrackedOutboundLink href={instagramUrl} eventName="Instagram Click" properties={{ source: "floating_button" }} ariaLabel="ZPHC Türkiye Instagram hesabını aç" className="flex h-11 w-11 items-center justify-center rounded-full bg-[radial-gradient(circle_at_30%_105%,#fdf497_0%,#fd5949_45%,#d6249f_60%,#285aeb_90%)] text-white shadow-[0_8px_24px_rgba(0,0,0,0.18)] transition-transform hover:scale-105 sm:h-14 sm:w-14">
      <InstagramIcon />
    </TrackedOutboundLink>
    <TrackedOutboundLink href={whatsappLink("Merhaba, ürün fiyatları ve stok durumu hakkında bilgi alabilir miyim?")} eventName="WhatsApp Click" properties={{ source: "floating_button" }} ariaLabel="WhatsApp'tan yazın" className="flex h-11 w-11 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_8px_24px_rgba(0,0,0,0.18)] transition-transform hover:scale-105 sm:h-14 sm:w-14">
      <WhatsappIcon />
    </TrackedOutboundLink>
  </div>
}
